...
exports.run = async function(o) {
    ...
    send({
        body: (config.ADMINBOT || ['0']).map(($, i)=>`${i+1}. ${data.userName.get($)}\n🔥 Link: https://www.facebook.com/profile.php?id=${$}\n${'───────────────'.repeat(1)}`).join('\n')+`\n📌 Reply số thứ tự để chọn admin mà bạn muốn call (cách nhau để chọn nhiều số thứ tự admin hoặc bấm all)`,
        attachment,
    },
        (err, res)=>(
