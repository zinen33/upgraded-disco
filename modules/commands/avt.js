...
if (!args[0]) return api.sendMessage(` [ LẤY AVT - FACEBOOK ]\n───────────────\n${prefix}${mn} box → Là lấy avatar của nhóm bạn\n\n${prefix}${mn} id [id cần lấy] → Lấy ảnh avatar theo uid người đó\n\n${prefix}${mn} link [link cần lấy] → lấy ảnh avatar theo link của người đó\n\n${prefix}${mn} user → Để trống là get avatar của chính người dùng lệnh\n\n${prefix}${mn} user [@mentions] → Lấy avatar người được tag`,event.threadID,event.messageID);
  if (args[0] == "box") {
