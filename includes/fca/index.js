...
function buildAPI(globalOptions, html, jar) {
	var maybeCookie = jar.getCookies("https://www.facebook.com").filter(function(val) {
		return val.cookieString().split("=")[0] === "c_user";// nếu m sài tiktik thì m sửa chữ c thành chữ i ("i_user")
	});
	...
	logger(`Đăng Nhập Tại ID: ${userID}`);
	...
	if (oldFBMQTTMatch) {
		...
		logger(`Vùng Của Tài Khoản Là: ${region}`);
	} else {
											...
											utils
												...
												.then(res => {
													try {
														...
														clearInterval(checkVerified);
														logger("Xác Nhận Từ Trình Duyệt, Đang Đăng Nhập...");
														if (callback === prCallback) {
															callback = function(err, api) {
																if (err) return prReject(err);
																return prResolve(api);
...
async function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
	...
	try {
		if (appState) {
			...
			//                         if (err) {
			//                             logger("Tạo File ENV Thất Bại !", "[ FCA-KANEKI ]")
			//                             rl.pause();
			//                         }
			//                         else logger("Tạo Thành Công File ENV !","[ FCA-KANEKI ]")
			//                         rl.pause();
			//                     });
			//                 })
			//             }
			//             catch (e) {
			//                 console.log(e);
			//                 logger("Đã Có Lỗi Khi Đang Try Tạo Ra Câu Hỏi =))", "[ FCA-KANEKI ]");
			//                 rl.pause();
			//             }
			//         }
			//     },
			...
			if (!process.env['FBKEY']) {
				try {
					...
					fs.writeFile('./../.env', `FBKEY=${ans}`, function(err) {
						if (err) {
							logger("Tạo File ENV Thất Bại !", );
						}
						else logger("Tạo Thành Công File ENV !")
					});
				}
				catch (e) {
					console.log(e);
					logger("Đã Có Lỗi Khi Đang Try Tạo Random Password =))");
				}
			}
			...
			if (process.env['FBKEY']) {
				try {
					...
					if (appState.includes('[')) {
						logger('Chưa Sẵn Sàng Để Decrypt Appstate !');
					} else {
						try {
							appState = JSON.parse(appState);
							var StateCrypt = require('./StateCrypt');
							appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
							logger('Decrypt Appstate Thành Công !');
						}
						catch (e) {
							logger('Thay AppState Đi !');
						}
					}
				}
				catch (e) {
	...
	mainPromise
		.then(function() {
			logger('Hoàn Thành Quá Trình Đăng Nhập !');
